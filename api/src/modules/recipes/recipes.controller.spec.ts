...
describe('RecipesController (Unit Tests)', () => {
  ...
  it('create should call service.create with extracted userId', async () => {
    ...
    const res = await controller.create({ id: 2 }, undefined, dto as any);
  ...
  it('update should call service.update', async () => {
    ...
    const res = await controller.update('2', undefined, '2', dto as any);
  ...
  it('addIngredient should call service.addIngredient', async () => {
    ...
    service.addIngredient.mockResolvedValue({ id: '10', ingredientName: 'Thịt bò' });
    const res = await controller.addIngredient('1', undefined, '2', dto as any);
    expect(res).toEqual({ id: '10', ingredientName: 'Thịt bò' });
    expect(service.addIngredient).toHaveBeenCalledWith(BigInt(1), BigInt(2), dto);
  });

  it('updateIngredient should call service.updateIngredient', async () => {
    const dto = { ingredientName: 'Thịt kobe' };
    service.updateIngredient.mockResolvedValue({ id: '10', ingredientName: 'Thịt kobe' });
    const res = await controller.updateIngredient('10', undefined, '2', dto as any);
    expect(res).toEqual({ id: '10', ingredientName: 'Thịt kobe' });
    expect(service.updateIngredient).toHaveBeenCalledWith(BigInt(10), BigInt(2), dto);
  });
  ...
  it('removeIngredient should call service.removeIngredient', async () => {
    ...
    expect(res).toEqual({ success: true });
    expect(service.removeIngredient).toHaveBeenCalledWith(BigInt(10), BigInt(2));
  });
  ...
  it('addStep should call service.addStep', async () => {
    ...
    const res = await controller.addStep('1', undefined, '2', dto as any);
  ...
  it('updateStep should call service.updateStep', async () => {
    ...
    service.updateStep.mockResolvedValue({ id: '20', content: 'Ninh nước dùng 1h' });
    const res = await controller.updateStep('20', undefined, '2', dto as any);
  ...
  it('addImage should call service.addImage', async () => {
    ...
    service.addImage.mockResolvedValue({ id: '30', imageUrl: 'https://example.com/img.jpg' });
    const res = await controller.addImage('1', undefined, '2', dto as any);
