...
describe('RecipesService (Unit Tests)', () => {
  ...
  describe('CRUD Recipe', () => {
    describe('create (CREATE Recipe)', () => {
      it('should create a new recipe for user', async () => {
        ...
        const dummyRecipe = { id: BigInt(100), userId: BigInt(1), ...dto, isPublic: true };
        prisma.recipe.create.mockResolvedValue(dummyRecipe);

        const res = await service.create(BigInt(1), dto as any);
    ...
    describe('findAll (READ Recipe List)', () => {
      it('should return paginated recipes filtering deletedAt: null', async () => {
        const dummyRecipes = [{ id: BigInt(1), title: 'Phở Bò', userId: BigInt(1) }];
    ...
    describe('findOne (READ Recipe Detail)', () => {
      it('should return recipe detail with ingredients, steps, images, and tags', async () => {
        const dummyRecipe = { id: BigInt(1), title: 'Phở Bò', userId: BigInt(1) };
        prisma.recipe.findFirst.mockResolvedValue(dummyRecipe);
        prisma.recipeIngredient.findMany.mockResolvedValue([{ id: BigInt(10), ingredientName: 'Bánh phở' }]);
        prisma.recipeStep.findMany.mockResolvedValue([{ id: BigInt(20), content: 'Trần bánh phở' }]);
        prisma.recipeImage.findMany.mockResolvedValue([{ id: BigInt(30), imageUrl: 'pho.jpg' }]);
        prisma.recipeTag.findMany.mockResolvedValue([{ recipeId: BigInt(1), tagId: BigInt(5) }]);
        prisma.tag.findMany.mockResolvedValue([{ id: BigInt(5), name: 'Ăn sáng' }]);
    ...
    describe('update (UPDATE Recipe)', () => {
      it('should update recipe if user is the owner', async () => {
        const dummyRecipe = { id: BigInt(1), userId: BigInt(2), title: 'Phở Bò' };
        prisma.recipe.findFirst.mockResolvedValue(dummyRecipe);
        prisma.user.findUnique.mockResolvedValue({ id: BigInt(2), role: 'USER' });
        prisma.recipe.update.mockResolvedValue({ ...dummyRecipe, title: 'Phở Bò Tái' });

        const res = await service.update(BigInt(1), BigInt(2), { title: 'Phở Bò Tái' });
      ...
      it('should throw ForbiddenException if user is not the owner nor admin', async () => {
        const dummyRecipe = { id: BigInt(1), userId: BigInt(2), title: 'Phở Bò' };
        prisma.recipe.findFirst.mockResolvedValue(dummyRecipe);
        prisma.user.findUnique.mockResolvedValue({ id: BigInt(3), role: 'USER' });

        await expect(service.update(BigInt(1), BigInt(3), { title: 'Hack' })).rejects.toThrow(ForbiddenException);
      });
    });
    ...
    describe('remove (DELETE / Soft Delete Recipe)', () => {
      it('should update deletedAt timestamp on recipe when removing', async () => {
        ...
        prisma.recipe.findFirst.mockResolvedValue(dummyRecipe);
        prisma.user.findUnique.mockResolvedValue({ id: BigInt(2), role: 'USER' });
        prisma.recipe.update.mockResolvedValue({ ...dummyRecipe, deletedAt: new Date() });
  ...
  describe('CRUD Ingredient', () => {
    beforeEach(() => {
      prisma.recipe.findFirst.mockResolvedValue({ id: BigInt(1), userId: BigInt(2) });
      prisma.user.findUnique.mockResolvedValue({ id: BigInt(2), role: 'USER' });
    });
    ...
    it('addIngredient (CREATE) should add ingredient to recipe', async () => {
      ...
      const res = await service.addIngredient(BigInt(1), BigInt(2), dto as any);
    ...
    it('updateIngredient (UPDATE) should update ingredient details', async () => {
      prisma.recipeIngredient.findUnique.mockResolvedValue({ id: BigInt(10), recipeId: BigInt(1) });
      prisma.recipeIngredient.update.mockResolvedValue({ id: BigInt(10), ingredientName: 'Thịt bò kobe' });

      const res = await service.updateIngredient(BigInt(10), BigInt(2), { ingredientName: 'Thịt bò kobe' });
    ...
    it('removeIngredient (DELETE) should remove ingredient from recipe', async () => {
      ...
      expect(res.success).toBe(true);
      expect(prisma.recipeIngredient.delete).toHaveBeenCalledWith({ where: { id: BigInt(10) } });
    });
  });
  ...
  describe('CRUD Step', () => {
    ...
    it('addStep (CREATE) should add cooking step to recipe', async () => {
      ...
      const res = await service.addStep(BigInt(1), BigInt(2), dto as any);
    ...
    it('updateStep (UPDATE) should update cooking step content', async () => {
      prisma.recipeStep.findUnique.mockResolvedValue({ id: BigInt(20), recipeId: BigInt(1) });
      prisma.recipeStep.update.mockResolvedValue({ id: BigInt(20), content: 'Ninh xương 30 phút' });

      const res = await service.updateStep(BigInt(20), BigInt(2), { content: 'Ninh xương 30 phút' });
    ...
    it('removeStep (DELETE) should remove step from recipe', async () => {
      ...
      expect(res.success).toBe(true);
      expect(prisma.recipeStep.delete).toHaveBeenCalledWith({ where: { id: BigInt(20) } });
    });
  });
  ...
  describe('CRUD Image', () => {
    ...
    it('addImage (CREATE) should add image to recipe', async () => {
      ...
      const res = await service.addImage(BigInt(1), BigInt(2), dto as any);
    ...
    it('removeImage (DELETE) should remove image from recipe', async () => {
      prisma.recipeImage.findUnique.mockResolvedValue({ id: BigInt(30), recipeId: BigInt(1) });
      prisma.recipeImage.delete.mockResolvedValue({});

      const res = await service.removeImage(BigInt(30), BigInt(2));
      expect(res.success).toBe(true);
      expect(prisma.recipeImage.delete).toHaveBeenCalledWith({ where: { id: BigInt(30) } });
    });
  });
});
