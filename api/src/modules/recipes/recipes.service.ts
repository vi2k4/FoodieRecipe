...
import { CreateIngredientDto, UpdateIngredientDto, CreateStepDto, UpdateStepDto, CreateImageDto } from './dto/sub-resources.dto';
import { RecipeDifficulty, RecipeImageType, Prisma } from '../../generated/prisma/client';
...
export class RecipesService {
  ...
  async findAll(query: QueryRecipeDto) {
    const { page = 1, limit = 10, search, categoryId, difficulty, isPublic, userId } = query;
  ...
  async findOne(id: bigint) {
    ...
    const [ingredients, steps, images, tags] = await Promise.all([
      this.prisma.recipeIngredient.findMany({ where: { recipeId: id }, orderBy: { displayOrder: 'asc' } }),
      this.prisma.recipeStep.findMany({ where: { recipeId: id }, orderBy: { stepNumber: 'asc' } }),
      this.prisma.recipeImage.findMany({ where: { recipeId: id }, orderBy: { displayOrder: 'asc' } }),
  ...
  async create(userId: bigint, dto: CreateRecipeDto) {
    if (dto.calories !== undefined && dto.calories !== null && Number(dto.calories) < 0) {
      throw new BadRequestException('Số lượng calo không được là số âm!');
    }
    if (dto.cookTime !== undefined && dto.cookTime !== null && Number(dto.cookTime) < 0) {
      throw new BadRequestException('Thời gian nấu không được là số âm!');
    }
    if (dto.servings !== undefined && dto.servings !== null && Number(dto.servings) < 1) {
  ...
  async update(id: bigint, userId: bigint, dto: UpdateRecipeDto) {
    const recipe = await this.prisma.recipe.findFirst({ where: { id, deletedAt: null } });
    if (!recipe) throw new NotFoundException('Recipe not found');
    ...
    const updated = await this.prisma.recipe.update({
      ...
      data: {
        ...
        calories: dto.calories !== undefined ? (dto.calories ? Math.max(0, Number(dto.calories)) : null) : undefined,
        cookTime: dto.cookTime !== undefined ? (dto.cookTime ? Math.max(0, Number(dto.cookTime)) : null) : undefined,
        difficulty: dto.difficulty as RecipeDifficulty,
        servings: dto.servings !== undefined ? (dto.servings ? Math.max(1, Number(dto.servings)) : 4) : undefined,
        thumbnail: dto.thumbnail,
        source: dto.source,
  ...
  async addIngredient(recipeId: bigint, userId: bigint, dto: CreateIngredientDto) {
  ...
  async updateStep(id: bigint, userId: bigint, dto: UpdateStepDto) {
    ...
    const updated = await this.prisma.recipeStep.update({
      where: { id },
      data: {
        stepNumber: dto.stepNumber !== undefined ? dto.stepNumber : step.stepNumber,
  ...
  private async checkOwnership(recipeId: bigint, userId: bigint) {
    const recipe = await this.prisma.recipe.findFirst({ where: { id: recipeId, deletedAt: null } });
    if (!recipe) throw new NotFoundException('Recipe not found');
    ...
    if (recipe.userId !== userId && !isAdmin) {
      throw new ForbiddenException('Bạn không phải là chủ sở hữu của công thức này!');
    }
  }
}
